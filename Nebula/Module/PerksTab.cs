...
public class PerksTab : InventoryTab
{
    ...
    private void OpenPerkDialog<P>(bool isCrewmate, Action<P?> adaptor, Predicate<P> duplicateChecker, IEnumerable<P> allPerks) where P : DisplayPerk
    {
        ...
        PerkDisplay GenerateDisplay()
        {
            ...
            obj.transform.localPosition = new Vector3((num % x - 5f) * 0.68f, 1.3f - num / x * 0.62f, -2f);
    ...
    private PerkDisplay[] AbilityPerkDisplays;
    private GameObject[] InvalidPerkMask;
    private PerkDisplay RolePerkDisplay;
    private TextMeshPro PerkNameText;
    private TextMeshPro PerkFlavor;

    public void Start()
    {
        var headerText = GameObject.Instantiate(PlayerCustomizationMenu.Instance.transform.GetChild(4).GetChild(0).gameObject.GetComponent<TextMeshPro>(), transform);
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            obj.transform.localPosition = new Vector3(0.9f * (i % 5 - 2), (i < 5 ? 0.9f : -2.3f) + (i % 2 == 1 ? 0f : 0.56f), 0f);
            ...
            int index = i;
            button.OnClick.AddListener((Action)(() => OpenPerkDialog(index < 5, (p) =>
            {
        ...
        button.OnClick.AddListener((Action)(() => OpenPerkDialog(false, (p) => PerkSaver.SetEquipedRolePerk(0, false, p), (p) => PerkSaver.GetEquipedRolePerk(0, false) == p, Perks.AllRolePerks.Values)));
        button.OnMouseOver.AddListener((Action)(() =>
        {
...
{
    private static PerksTab PerksTab;
    private static GameObject PerksGroup;
    private static GameObject PerksHeader;


    [HarmonyPatch(typeof(PlayerCustomizationMenu), nameof(PlayerCustomizationMenu.Start))]
    private class AddPerksTabPatch
    {
        private static Sprite perksTabSprite;
