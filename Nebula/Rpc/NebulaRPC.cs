...
public class RemoteProcessBase
{
    ...
    public static Dictionary<int, RemoteProcessBase> AllNebulaProcess = new();
    ...
    public RemoteProcessBase(string name)
    {
        ...
        if (AllNebulaProcess.ContainsKey(Hash)) Info("NebulaRPC", $"Identifier conflict has been occured at \"{Name}\"");
...
public class DivisibleRemoteProcess<Parameter, DividedParameter> : RemoteProcessBase
{
    ...
    public DivisibleRemoteProcess(string name, Action<Parameter, Action<DividedParameter>> sender, Action<MessageWriter, DividedParameter> dividedSender, Func<MessageReader, DividedParameter> receiver, Process process)
    : base(name)
    {
        Sender = sender;
...
internal class NebulaRPCHandlerPatch
{
