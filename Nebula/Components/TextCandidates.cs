...
public class TextCandidates : MonoBehaviour
{
    ...
    private ISpriteLoader backgroundSprite = new AssetSpriteLoader(AssetLoader.NebulaMainAsset, "TextCandidateBack");
    private ISpriteLoader highlightSprite = new AssetSpriteLoader(AssetLoader.NebulaMainAsset, "TextCandidateHighlight");
    public void Awake()
    {
    ...
    public void UpdateCandidates(TextCandidate[]? candidates)
    {
        ...
        foreach (TextCandidate candidate in candidates)
        {
            ...
            Candidates[i].Show();
            float num = Candidates[i].SetText(candidate, i * Height * (ExtendsDownward ? -1f : 1f));
            if (num > width) width = num;
            i++;
        }
        Background.gameObject.transform.localPosition = new Vector3(0.1f + width * 0.5f, Height * (i - 1) * 0.5f * (ExtendsDownward ? -1f : 1f), 0f);
        Background.size = new Vector2(width + 0.4f, Height * i + 0.2f);
